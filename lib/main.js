...
function run() {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const { repo: ctxRepo, ref, sha } = github.context;
            /*
              this allows for `repository` to be
              - null/undefined (defaults to current action repo)
              - repo (defaults owner to same as current action)
              - owner/repo
            */
            let [repository = ctxRepo.repo, owner = ctxRepo.owner] = (((_a = core.getInput("repository")) === null || _a === void 0 ? void 0 : _a.split("/")) || []).reverse();
            const step = core.getInput("step", { required: true });
            const coreArgs = {
                autoInactive: core.getInput("auto_inactive") !== "false",
                logsURL: core.getInput("logs") ||
                    `https://github.com/${owner}/${repository}/commit/${sha}/checks`,
                ...
                case "start":
                    {
                        ...
                        if (!args.noOverride) {
                            yield deactivate_1.default(client, owner, repository, args.environment);
                        }
                        if (!deploymentID) {
